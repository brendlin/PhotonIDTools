...
struct photonID
{
  photonID(int n_et,int n_eta) {
    net = n_et;
    neta = n_eta;
    CutHadLeakage = new Float_t[net*neta];
    Reta37        = new Float_t[net*neta];
    Rphi33        = new Float_t[net*neta];
    weta2         = new Float_t[net*neta];
    fracm         = new Float_t[net*neta];
    wtot          = new Float_t[net*neta];
    w1            = new Float_t[net*neta];
    deltae        = new Float_t[net*neta];
    DEmaxs1       = new Float_t[net*neta];
    isolation     = "";
    EtaBinThresholds       = new Float_t[neta];
    EtBinThresholds        = new Float_t[net-1];
  ...
  Float_t* EtaBinThresholds;
  Float_t* EtBinThresholds;
  ...
  void Set_CutHadLeakage(Float_t* c){ do_CutHadLeakage = true; for (int i=0; i < net*neta; i++) CutHadLeakage[i] = c[i]; }
  void Set_Reta37       (Float_t* c){ do_Reta37        = true; for (int i=0; i < net*neta; i++) Reta37[i]        = c[i]; }
  void Set_Rphi33       (Float_t* c){ do_Rphi33        = true; for (int i=0; i < net*neta; i++) Rphi33[i]        = c[i]; }
  void Set_weta2        (Float_t* c){ do_weta2         = true; for (int i=0; i < net*neta; i++) weta2[i]         = c[i]; }
  void Set_fracm        (Float_t* c){ do_fracm         = true; for (int i=0; i < net*neta; i++) fracm[i]         = c[i]; }
  void Set_wtot         (Float_t* c){ do_wtot          = true; for (int i=0; i < net*neta; i++) wtot[i]          = c[i]; }
  void Set_w1           (Float_t* c){ do_w1            = true; for (int i=0; i < net*neta; i++) w1[i]            = c[i]; }
  void Set_deltae       (Float_t* c){ do_deltae        = true; for (int i=0; i < net*neta; i++) deltae[i]        = c[i]; }
  void Set_DEmaxs1      (Float_t* c){ do_DEmaxs1       = true; for (int i=0; i < net*neta; i++) DEmaxs1[i]       = c[i]; }

  // No lower bin edge (0); Yes upper bin edge (2.47)
  void Set_EtaBinThresholds(Float_t* c){for (int i=0; i < neta; i++) EtaBinThresholds[i] = c[i]; }
  // No lower bin edge (0); No upper bin edge (inf).
  void Set_EtBinThresholds (Float_t* c){for (int i=0; i < net-1 ; i++) EtBinThresholds [i] = c[i]; }
};
                      ...
                      bool doIsolationPreselection = false)
{
  ...
  bool y_iso_FixedCutLoose,y_IsTight;
  ...
  for (Long64_t i = 0; i < nentries; i++) {
    ...
    bool passNum = true;
    passNum = passNum && y_e277 >= 0.1; // The typical e277 cut, hard-coded in here.
    passNum = passNum && y_f1 >= 0.005; // The typical f1   cut, hard-coded in here.
